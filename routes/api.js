  ...
  MongoClient.connect(CONNECTION_STRING, function(err, db) {
    ...
    app.route('/api/stock-prices').get(function(req, res) {
      ...
      stockAPI(stock)
        .then(data => {
          // Handle one stock
          // console.log("here is datas", data)
          if (!Array.isArray(data)) {
                  ...
                  if (r.result.ok) {
                    res.json({
                      stockData: {
                        ...
                        likes: validVoteIp ? doc.stockData.likes + 1 : doc.stockData.likes
                      }
                    });
                  }
                });
            ...
            dbController.findTwo(data).then(docs => {
              ...
              if (docs.length === 2) {
                ...
                const firstVote = !docs[0].stockData.voters.includes(ip) && like ? ip : false;
                const secondVote = !docs[1].stockData.voters.includes(ip) && like ? ip : false;
                const firstStock = docs[0].stockData.stock === data[0].symbol ? data[0] : data[1];
                const secondStock = docs[0].stockData.stock === data[0].symbol ? data[1] : data[0];
